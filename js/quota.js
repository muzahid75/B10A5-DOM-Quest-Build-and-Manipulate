document.getElementById("quota-donate-btn").addEventListener('click', function (event) {
    ...
    const inputFieldAdd = getInputFieldById("quota-input-field")
    ...
    const textFieldPrevious = getTextFieldById("quota-previous-balance");

    const newAmount = textFieldPrevious + inputFieldAdd;
    document.getElementById("quota-previous-balance").innerText = newAmount;
    ...
    newElement.innerHTML = `
        ...
        <p>${inputFieldAdd} taka donated for donate flood at quota bangladesh</p>
        <p>${Date()}</p>
        </div>`;
