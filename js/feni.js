document.getElementById("feni-donate-btn").addEventListener('click',function(event){
    ...
    const inputFieldAdd = getInputFieldById("feni-input-field")
    ...
    const textFieldPrevious =getTextFieldById("feni-previous-balance");

    const newAmount=textFieldPrevious+inputFieldAdd;
    document.getElementById("feni-previous-balance").innerText=newAmount;
    ...
    newElement.innerHTML = `
        ...
        <p>${inputFieldAdd} taka donated for donate flood at feni bangladesh</p>
        <p>${Date()}</p>
        </div>`;
